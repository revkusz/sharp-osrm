...
/// <summary>
/// Managed configuration for the OSRM MLD customization pipeline stage.
/// Mirrors the fields of <c>SharposrmCustomizerConfig</c>.
/// </summary>
public sealed class CustomizerConfig
{
    /// <summary>
    ...
    /// </summary>
    internal NativePipelineConfigScope<NativeCustomizerConfig> ToNative()
    {
        if (string.IsNullOrEmpty(BasePath))
            throw new ArgumentNullException(nameof(BasePath), "BasePath is required.");

        var scope = new NativePipelineConfigScope<NativeCustomizerConfig>();
        ...
        scope.Config = new NativeCustomizerConfig
        {
    ...
    private static (IntPtr arrayPtr, int count) AllocateStringArray(
        NativePipelineConfigScope<NativeCustomizerConfig> scope, string[]? strings)
    {
