...
/// <summary>
/// Managed parameters for the OSRM Route service.
/// Convert to a native params struct via <see cref="ToNative"/> for interop calls.
/// </summary>
public sealed class RouteParameters
{
    /// <summary>
    /// Coordinates for the route query (longitude-first OSRM convention).
    ...
    /// or <see cref="Bearing.None"/> as a convenience.
    /// If provided, the count must match <see cref="Coordinates"/>.
    /// </summary>
    ...
    /// These are the <c>hint</c> values from a previous response's waypoints.
    /// If provided, the count must match <see cref="Coordinates"/>.
    ...
    /// <see cref="NativeRouteParamsScope"/> to free all unmanaged allocations.
    /// The scope must not be disposed until the native call returns.
    /// </para>
    /// </summary>
    /// <returns>
    /// A <see cref="NativeRouteParamsScope"/> containing the native struct and
    ...
    /// </exception>
    internal NativeRouteParamsScope ToNative()
    {
        if (Coordinates is null || Coordinates.Count < 2)
        {
            throw new ArgumentException(
                "At least 2 coordinates are required for a route query.",
                nameof(Coordinates));
        }

        var scope = new NativeRouteParamsScope();
        ...
        if (Bearings is not null && Bearings.Count > 0)
        {
            ...
            int bearingBytes = bearingCount * 2 * sizeof(short); // 2 shorts per coordinate
...
/// <summary>
/// Owning handle for a <see cref="NativeRouteParams"/> and its allocated coordinate/radius arrays.
/// Dispose to free all unmanaged memory. Must not be disposed until the native call returns.
/// </summary>
internal sealed class NativeRouteParamsScope : IDisposable
{
    private readonly List<IntPtr> _allocations = new();
    private bool _disposed;

    public NativeRouteParams Params;
